...
import com.entrobus.credit.vo.product.InterestVo;
...
public class ProductApiController extends ManagerBaseController {
    ...
    public WebResult getProductInfo() {
        ...
        List<InterestVo> repaymentTerms = new ArrayList<>();
        for (int i = 0; i < termList.size(); i++) {
            LoanPeriodsRateVo term = termList.get(i);
            InterestVo ivo = new InterestVo();
            terms.add(term.getPeriods() + "");
            ivo.setTerm(term.getPeriods());
            ivo.setAverageCapitalInterest(term.getMonthEqualRate());//等额本息
            ivo.setInterestFirst(term.getInterestCapitalRate());//先息后本
            repaymentTerms.add(ivo);
        }
